...
def evaluate_model(model, testRatings, testNegatives, K):
    ...
    testSamples = testSamples.flatten()
    userSamples = np.asarray(range(shape[0])).repeat(shape[1])
    test_genre_samples = item_to_genre(testSamples, data_size='ml-1m').values
    test_genre_samples = np.nan_to_num(test_genre_samples)
    ...
    predictions = model.predict([userSamples, test_genre_samples], batch_size=256)
